...
CLIProxyAPI **原生 Go 插件**（当前版本 **0.2.5**）：仅针对 **xAI** 登录凭证做额度/死号管控、主动巡查，并提供管理 UI 与用量统计。
...
plugins:
  configs:
    cpa-xai-quota-guard:
      ...
      patrol_model: "grok-4.5-build-free"  # 主探测模型；建议免费档
      patrol_auto_model_switch: false   # true=402 时自动试该凭证其它模型
```
...
- 200/429 → 存活（spending 冷却号则自动启用）
- 402 spending-limit → 冷却（可选自动换模型后再定）
- 403/401 → DELETE
...
| 维度 | 说明 |
|------|------|
| 含义 | 当前凭证对**所用模型**无可用积分/订阅额度，或免费档不适用该模型 |
| 不是 | 权限死号（403）、凭证失效（401） |
| 与 429 | 429 free-usage = 滚动 24h 免费额度用尽；402 = spending/订阅积分阻塞 |
| usage 路径 | 业务请求 402 → `plugin_auto` 冷却（signal=`spending_limit`） |
| 巡查主模型 | `patrol_model`（默认 `grok-4.5-build-free`） |
| 自动换模 | `patrol_auto_model_switch=true` 时：主模型 402 → 拉该凭证 `/models` → 依次试备用（优先 free）→ 仍 402 才冷却 |
| 关自动换模 | 仅测 `patrol_model`，402 → 冷却禁用 |
| 恢复 | 巡查得到 200 或 429 free-usage → 自动启用；tick 软上限约 24h 也可启 |
| 改配置后 | UI「仅复查冷却号」或 `POST .../patrol/spending` 只扫 spending 冷却账号 |

**误伤提示**：用付费模型（如 `grok-3`）全量巡查，易对仅有 free 档的号统一 402。请用 free 档主模型，或开启自动换模。
